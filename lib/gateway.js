...
// of the tree.
async function handleStatic (req, res, segments, uiRoot) {
  ...
  // Containment is decided on the resolved path, so traversal is rejected even
  // when realpath is unavailable.
  if (target !== root && !target.startsWith(root + path.sep)) {
    throw httpError(400, 'Invalid path')
  }

  // realpath additionally defeats symlink escapes, but it is not implemented on
  // every filesystem the gateway runs on — under Pear the app directory is
  // virtualized and realpath throws, which used to 404 the whole UI. Treat it
  // as a bonus check rather than a requirement.
  let real = target
  try {
...
function createGateway (node, { port = 8433, host = '127.0.0.1', ui = true, uiRoot = path.resolve(__dirname, '..', 'ui') } = {}) {
  const jobs = new Map()
  let nextJobId = 1
  ...
  const server = http.createServer(async (req, res) => {
    ...
    try {
      ...
      if (parts.length === 0) {
        ...
        if (ui) return await handleStatic(req, res, ['index.html'], uiRoot)
        return sendJSON(res, 200, await node.state())
      }
      ...
      if (parts[0] === 'assets') {
        return await handleStatic(req, res, parts, uiRoot)
      }
      ...
      if (ui && (req.method === 'GET' || req.method === 'HEAD')) {
        return await handleStatic(req, res, ['index.html'], uiRoot)
      }
